...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if not event.is_directory and self._should_process(event.src_path):
            ...
            asyncio.create_task(self.pending_events.put(self._create_event("created", event.src_path)))

    def on_modified(self, event):
        if not event.is_directory and self._should_process(event.src_path):
            logger.info(f"File modified: {event.src_path}")
            asyncio.create_task(self.pending_events.put(self._create_event("modified", event.src_path)))

    def on_deleted(self, event):
        if not event.is_directory and self._should_process(event.src_path):
            logger.info(f"File deleted: {event.src_path}")
            asyncio.create_task(self.pending_events.put(self._create_event("deleted", event.src_path)))

    def on_moved(self, event):
        if not event.is_directory and self._should_process(event.src_path):
            logger.info(f"File moved: {event.src_path} -> {event.dest_path}")
            asyncio.create_task(self.pending_events.put(self._create_event("moved", event.src_path, event.dest_path)))
...
class FileWatcherService:
    ...
    def save_config(self):
        ...
        try:
            ...
            data = {"folders": [asdict(folder) for folder in self.folders.values()]}
