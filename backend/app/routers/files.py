...
async def list_files(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    results = await client.scroll(collection_name="files", limit=1000, with_payload=True, with_vectors=False)
...
async def get_file(file_id: str, request: Request, current_user: dict = Depends(get_current_user)):
    ...
    results = await QdrantManager.safe_retrieve(
        client, collection_name="files", ids=[file_id], with_payload=True, with_vectors=False
    )
...
async def _reindex_file_task(file_id: str, file_path: str):
    ...
    try:
        ...
        # Broadcast event
        await websocket_manager.broadcast(WebSocketEvents.file_reindexed(file_id, file_path))
...
async def _remove_file_from_index(file_path: str):
    ...
    if results[0]:
        await client.delete(collection_name="files", points_selector=[results[0][0].id])
...
async def _update_file_path(old_path: str, new_path: str):
    ...
    results = await client.scroll(
        collection_name="files",
        scroll_filter={"must": [{"key": "path", "match": {"value": old_path}}]},
        limit=1,
        with_payload=True,
        with_vectors=False,
    )
    ...
    if results[0]:
        ...
        await client.set_payload(collection_name="files", payload=payload, points=[file_id])
